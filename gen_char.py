...
class Font2Image(object):
    ...
    def do(self, font_path, char, rotate=0):
        ...
        useWhite = False
        # 黑色背景
        img = Image.new("RGB", (self.width, self.height), "white" if useWhite else "black")
        draw = ImageDraw.Draw(img)
        font = ImageFont.truetype(font_path, int(self.height * 0.8),)
        # 白色字体
        draw.text((int(self.height*0.1), int(self.height*0.1)), char, (0, 0, 0) if useWhite else (255, 255, 255),
                  font=font)
        if rotate != 0:
...
def args_parse():
    ...
    parser.add_argument('--out_dir', dest='out_dir',
                        default='data/cn', required=False,
                        help='write a caffe dir')
    parser.add_argument('--font_dir', dest='font_dir',
                        default='chinese_fonts', required=False,
